...
void connect_sockets(Fid_t sock1, Fid_t lsock, Fid_t* sock2, port_t port)
{
	...
	accept_flag=1;
	Tid_t t1,t2;
	t1 = Exec(accept_thread, 0, NULL);
	t2 = Exec(connect_thread, 0, NULL);
	ASSERT(t1!=NOTHREAD);
	ASSERT(t2!=NOTHREAD);
	ASSERT(WaitChild(t1, NULL)!=NOPROC);
	ASSERT(WaitChild(t2, NULL)!=NOPROC);
	accept_flag=0;
...
/*	Tid_t t = CreateThread(accept_connection, 0, NULL);
	...
	Tid_t t = Exec(accept_connection, 0, NULL);
	ASSERT(Connect(cli, 100, 1000)==-1);
